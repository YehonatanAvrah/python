...
class Register(tkinter.Toplevel):
    ...
    def create_gui(self):
        ...
        self.SucRegLbl = Label(self, textvariable=self.SucReg, font=self.LblFont, background="yellow", width=35,
                               height=3)
        ...
        self.canvas.create_window(130, 250, window=self.button_reg)

        self.btn_close = Button(self, text="Previous Window", command=self.close, background="red", font=self.LblFont)
        self.canvas.create_window(280, 250, window=self.btn_close)
        ...
        self.canvas.create_window(545, 80, window=self.ShowHidePass)

        self.ShowHideConfirmPass = Button(self, image=self.ShowEye, command=self.toggle_confirm_pswrd, font=self.LblFont)
        self.canvas.create_window(545, 130, window=self.ShowHideConfirmPass)
